...
- [Ảnh có khoanh số](images/annotated/): 22 ảnh chụp giao diện có dải nhãn bên phải và đường nối mảnh tới vị trí tương ứng, không có hộp chữ che giao diện.
...
`build_visual_handbook.py` thêm vòng tròn đánh số, dải nhãn bên phải và đường nối mảnh lên bản sao của ảnh chụp trong `images/annotated/`. Script in PDF kiểm tra ảnh tải đủ và nội dung không đụng chân trang trước khi tạo PDF. `verification.json` ghi kết quả theo từng trang.
