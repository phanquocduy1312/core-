...
@section('content')
    ...
    <div class="relative overflow-hidden mb-6 bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-xl shadow-sm border border-slate-700/50">
        <div class="px-6 py-4">
            <h4 class="text-xl font-bold mb-1 text-white">{{ __('admin.blog_categories.create') }}</h4>
    ...
    <form method="POST" action="{{ route('admin.post-categories.store') }}" class="admin-form-with-sticky-actions space-y-6" enctype="multipart/form-data">
        @csrf
