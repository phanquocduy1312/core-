...
@section('content')
    ...
    <div class="relative overflow-hidden mb-6 bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-xl shadow-sm border border-slate-700/50">
        <div class="px-6 py-4">
            <h4 class="text-xl font-bold mb-1 text-white">Sửa chương trình: {{ $promotion->name }}</h4>
    ...
    <form method="POST" action="{{ route('admin.promotions.update', $promotion) }}" class="admin-form-with-sticky-actions space-y-6">
        @csrf
        @method('PUT')
