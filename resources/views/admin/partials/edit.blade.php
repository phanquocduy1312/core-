...
@section('title', 'Chỉnh sửa khối dùng chung')

@section('content')
    <form method="POST" action="{{ route('admin.partials.update', $page) }}" id="page-builder-form" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="divide-y divide-gray-150 border border-gray-200 rounded-lg overflow-hidden">
                @foreach($revisions as $revision)
                    <div class="px-4 py-3 bg-white flex flex-wrap items-center justify-between gap-3 text-sm hover:bg-gray-50/50 transition-colors">
                        ...
                        <form method="POST" action="{{ route('admin.partials.revisions.restore', [$page, $revision]) }}">
                            @csrf
