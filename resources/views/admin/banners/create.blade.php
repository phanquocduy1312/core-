...
@section('content')
    ...
    <div class="relative overflow-hidden mb-6 bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-xl shadow-sm border border-slate-700/50">
        <div class="px-6 py-4 flex flex-wrap items-center justify-between gap-4">
            <div>
                <h4 class="text-xl font-bold mb-1 text-white">{{ __('admin.banners.create') }}</h4>
                <nav class="flex text-sm text-slate-350" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-2">
                        ...
                        <li>
                            <div class="flex items-center">
                                <iconify-icon icon="solar:alt-arrow-right-linear" class="mx-1 text-slate-500"></iconify-icon>
                                <span class="text-slate-400">{{ __('admin.banners.create') }}</span>
                            </div>
                        </li>
                    </ol>
                </nav>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 max-w-4xl">
        <form action="{{ route('admin.banners.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <!-- Banner Image Upload -->
            <div>
                <label class="block mb-2 text-sm font-semibold text-gray-900">{{ __('admin.banners.fields.image') }} <span class="text-red-500">*</span></label>
                <div class="p-6 border-2 border-dashed border-gray-300 rounded-xl bg-gray-50 hover:bg-gray-100/50 transition-colors text-center relative cursor-pointer">
                    ...
                    <h6 class="text-sm font-bold text-gray-900 mb-0.5">Chọn tập tin ảnh banner</h6>
                    <p class="text-2xs text-gray-400">Chấp nhận JPG, PNG, WEBP tối đa 2MB</p>
                    ...
                    <div id="imagePreviewContainer" class="mt-4 hidden flex justify-center">
                        <img id="imagePreview" src="" alt="Preview" class="max-h-48 rounded-lg shadow-sm border border-gray-200 object-contain">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" class="block w-full p-2.5 text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-primary focus:border-primary focus:outline-none transition-colors" id="title" name="title" value="{{ old('title') }}" placeholder="Nhập tiêu đề hoặc mô tả ngắn">
            </div>
            ...
            <div>
                ...
                <input type="url" class="block w-full p-2.5 text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-primary focus:border-primary focus:outline-none transition-colors" id="link_url" name="link_url" value="{{ old('link_url') }}" placeholder="https://example.com/san-pham">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" class="block w-full p-2.5 text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-primary focus:border-primary focus:outline-none transition-colors" id="sort_order" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" required>
                </div>
            </div>
            ...
            <div class="flex items-center pt-2">
                ...
                <input class="w-4 h-4 text-primary bg-gray-100 border-gray-300 rounded focus:ring-primary cursor-pointer" type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', '1') == '1' ? 'checked' : '' }}>
